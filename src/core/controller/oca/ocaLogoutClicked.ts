...
export async function ocaLogoutClicked(controller: Controller, _request: EmptyRequest): Promise<Empty> {
	try {
		await storeSecret(controller.context, "ocaAccessToken", undefined)
		...
		const defaultProvider: ApiProvider = "openrouter"
		await updateGlobalState(controller.context, "apiProvider", defaultProvider)
